...
Dagster only builds docker images for `linux/amd64`, which cannot run on arm. This repo contains a dockerfile derived from the official image such that it can be used in a helm deployment to k8s on arm machines, and has some small optimizations to reduce the image size.
...
## Using this Dockerfile for other deployment targets

This Dockerfile builds images for kubernetes, and omits cloud-specific packages (AWS, Azure, GCP) to keep image size small. These lines can be uncommented in the Dockerfile. For example, when you want to deploy directly to AWS, you can uncomment `dagster-aws`.
